.PHONY: all infra image build push deploy destroy version-bump
profile="eugene.st"
region=eu-west-1
...
deploy:
	cat "k8s.yaml.template" | sed "s/{{APP_VERSION}}/$(app_version)/g" | sed "s/{{AWS_ACCOUNT_ID}}/$(aws_account_id)/g" | sed "s/{{REGION}}/$(region)/g" | AWS_PROFILE=$(profile) KUBECONFIG=$(kubeconfig) kubectl apply -f -
...
destroy:
	cat "k8s.yaml.template" | sed "s/{{APP_VERSION}}/$(app_version)/g" | sed "s/{{AWS_ACCOUNT_ID}}/$(aws_account_id)/g" | sed "s/{{REGION}}/$(region)/g" | AWS_PROFILE=$(profile) KUBECONFIG=$(kubeconfig) kubectl delete -f - && echo "hit!" || echo "miss!"
